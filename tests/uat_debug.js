 ...
 *
 * Tests real user workflows end-to-end via the API backend
 * and validates frontend HTML/JS structure.
 *
 * UAT Scenarios:
 ...
 *   UAT-31  Altar Call (sidebar register badge, CSS hover)
...
async function runUAT() { console.log('RUNUAT_STARTED');
  ...
  await test('Sidebar renders rank and streak UI', () => {
    const js = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(js.includes('sidebarRank'));
    assert(js.includes('streakCounter'));
    assert(js.includes('updateRank'));
    assert(js.includes('updateStreak'));
  });
  ...
  await test('sidebar.js uses XP-based rank from ScriptoriumCore', () => {
    const sidebar = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(sidebar.includes('computeRankFromXp'), 'sidebar.js missing computeRankFromXp delegation');
    assert(sidebar.includes('getTotalXp'), 'sidebar.js missing getTotalXp call');
    assert(sidebar.includes('getRankProgress'), 'sidebar.js missing getRankProgress');
  });

  await test('sidebar.js displays XP progress bar', () => {
    const sidebar = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(sidebar.includes('nextXp'), 'sidebar.js missing next rank XP tracking');
    assert(sidebar.includes('XP'), 'sidebar.js missing XP label');
  });
  ...
  await test('map.html stores journey completion in localStorage', () => {
    ...
    assert(map.includes('scriptorium_journeys'), 'Missing journey completion tracking');
    assert(map.includes('_completionMarked'), 'Missing journey completion marker');
  });
  ...
  await test('sidebar.js has sidebar-register badge', () => {
    const sidebar = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(sidebar.includes('sidebar-register'), 'Missing register badge element');
    assert(sidebar.includes('register-star'), 'Missing register star icon');
    assert(sidebar.includes('register-text'), 'Missing register text');
    assert(sidebar.includes('REGISTER'), 'Missing REGISTER label');
  });
  ...
  await test('sidebar-register CSS transforms on hover', () => {
    const sidebar = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(sidebar.includes('.sidebar-register'), 'Missing sidebar-register CSS');
    assert(sidebar.includes('.sidebar-register:hover'), 'Missing hover state CSS');
    assert(sidebar.includes('border-color:#d4af37'), 'Missing gold border on hover');
  });
  ...
  await test('typology.html has clickable scripture references', () => {
    const html = fs.readFileSync(path.join(ROOT, 'typology.html'), 'utf8');
    assert(html.includes('openScriptureRef') || html.includes('data-ref'), 'Missing scripture reference handler');
  });
  ...
  await test('ethiopian-canon.html has clickable book cards with overlay', () => {
    const html = fs.readFileSync(path.join(ROOT, 'ethiopian-canon.html'), 'utf8');
    assert(html.includes('book-card') || html.includes('bookCard'), 'Missing book cards');
  });
  ...
  await test('map.html has step navigation (prev/next)', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('currentStepIndex'), 'Missing step navigation logic');
  });

  await test('map.html defines updateJourneyStep function', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('updateJourneyStep'), 'Missing journey step updater');
  });

  await test('map.html has place sidebar panel', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('showSidebar'), 'Missing place sidebar function');
  ...
  await test('map.html uses getPeopleWhoLived for place info', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('getPeopleWhoLived'), 'Missing people-who-lived data lookup');
  });

  await test('map.html uses calcDistance for distances', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('calcDistance'), 'Missing distance calculation');
  });

  await test('map.html has archaeology vault integration', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('ARCHAEOLOGY_VAULT'), 'Missing archaeology vault data integration');
  });
  ...
  await test('map.html journey completion triggers localStorage', () => {
    const html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('scriptorium_journeys'), 'Missing journey completion localStorage key');
    assert(html.includes('isLastStop') || html.includes('completed'), 'Missing completion detection');
  });
  ...
  await test('map.html has Ethiopian canon (crimson) theme variant CSS', () => {
    var html = fs.readFileSync(path.join(ROOT, 'map.html'), 'utf8');
    assert(html.includes('theme-ethiopian'), 'Missing Ethiopian theme variant');
  });
  ...
  await test('tabernacle.html has spatial audio engine', () => {
    var html = fs.readFileSync(path.join(ROOT, 'tabernacle.html'), 'utf8');
    assert(html.includes('initTabAudio'), 'Missing audio init function');
    assert(html.includes('activateZoneAudio'), 'Missing zone audio activator');
    assert(html.includes('AudioContext'), 'Missing Web Audio API');
  });
  ...
  await test('tabernacle.html defines ZONE_DATA with vessels and typology', () => {
    var html = fs.readFileSync(path.join(ROOT, 'tabernacle.html'), 'utf8');
    assert(html.includes('ZONE_DATA'), 'Missing zone data object');
    assert(html.includes('typology'), 'Missing typology data');
    assert(html.includes('antitype'), 'Missing antitype field');
    assert(html.includes('scripture'), 'Missing typology scripture reference');
  });

  await test('tabernacle.html has dynamic lighting (flicker/pulse)', () => {
    var html = fs.readFileSync(path.join(ROOT, 'tabernacle.html'), 'utf8');
    assert(html.includes('addDynLight'), 'Missing dynamic light function');
    assert(html.includes('dynamicLights'), 'Missing dynamicLights array');
  });

  await test('tabernacle.html has floor plan SVG overlay', () => {
    var html = fs.readFileSync(path.join(ROOT, 'tabernacle.html'), 'utf8');
    assert(html.includes('buildFloorPlan'), 'Missing floor plan builder function');
    assert(html.includes('floor-plan-overlay'), 'Missing floor plan overlay element');
  ...
  await test('tabernacle.html responsive at 768px breakpoint', () => {
    var html = fs.readFileSync(path.join(ROOT, 'tabernacle.html'), 'utf8');
    assert(html.includes('@media (max-width:768px)'), 'Missing mobile breakpoint');
    assert(html.includes('flex-direction:column'), 'Missing mobile column layout');
  });
  ...
  await test('sidebar.js links to login and register auth pages', () => {
    var js = fs.readFileSync(path.join(ROOT, 'assets/js/sidebar.js'), 'utf8');
    assert(js.includes('login.html'), 'Missing login link');
    assert(js.includes('register.html'), 'Missing register link');
  });
  ...
  await test('sidebar.js nav links include all major pages', () => {
    ...
    var targets = ['covenant-map.html','map.html','genealogy.html','tabernacle.html','typology.html','onomasticon.html','ethiopian-canon.html','scribes-chamber.html','plans.html','challenges.html','comparison-mode.html','settings.html','login.html','register.html'];
  ...
  await test('stub redirect pages link to placeholder or have meta refresh', () => {
    ['narthex.html', 'sanctum-3d.html', 'archive.html', 'prophetic-mesh.html'].forEach(function(f) {
      ...
      var hasUrl = html.includes('placeholder.html') || html.includes('http-equiv="refresh"');
      assert(hasUrl, f + ' missing redirect to placeholder');
    });
  });
...
console.log('LISTEN_CALLBACK_FIRED'); server = app.listen(PORT, () => {
  runUAT()
